globals [
  infection-chance         ;; Probabilidad de transmisión durante el contacto.
  incubation-period        ;; Tiempo hasta que aparecen los síntomas.
  mortality-rate           ;; Tasa de mortalidad del ébola.
  initial-infection-rate   ;; Porcentaje de la población infectada al principio.
  symptoms-show            ;; Tiempo hasta que una persona infectada muestra síntomas.
  slider-check-1           ;; Variables temporales para verificar cambios en los sliders.
  slider-check-2
  slider-check-3
  slider-check-4
  slider-check-5           ;; Variable temporal para el slider de aislamiento.
  total-population         ;; Conteo de la población total.
  infected-population      ;; Conteo de la población infectada.
  symptomatic-population   ;; Conteo de la población sintomática.
  isolated-population      ;; Conteo de la población aislada.
]

turtles-own [
  infected?                ;; Si la persona está infectada.
  symptomatic?             ;; Si la persona muestra síntomas.
  days-infected            ;; Días desde que la persona fue infectada.
  known?                   ;; Si la infección es conocida (y `infected?` debe ser verdadero).
  infection-length         ;; Tiempo que la persona ha estado infectada.
  coupled?                 ;; Si la persona está en una pareja activa.
  couple-length            ;; Tiempo que la persona ha estado en una pareja.
  coupling-tendency        ;; Probabilidad de que la persona forme una pareja.
  fluids-contact           ;; Porcentaje de probabilidad de que la persona use protección.
  test-frequency           ;; Número de veces que la persona se hará pruebas por año.
  partner                  ;; Pareja actual de la persona en una relación.
  isolated?                ;; Si la persona está aislada.
  isolation-time           ;; Tiempo que la persona ha estado en aislamiento.
]
...
to setup-globals
  set infection-chance 90           ;; Ajusta según datos del ébola.
  set incubation-period 15          ;; Promedio del período de incubación del ébola en días.
  set mortality-rate 50             ;; Tasa de mortalidad en porcentaje.
  set slider-check-2 average-coupling-tendency
  set slider-check-3 average-fluids-contact
  set slider-check-4 average-test-frequency
  set slider-check-5 isolation-period
  set total-population 0
  set infected-population 0
  set symptomatic-population 0
end

;; Configura las tortugas (personas) en el modelo.
to setup-people
  ...
  [
    ...
    ;; 2.5% de las personas comienzan infectadas, pero no lo saben.
    ...
    assign-fluids-contact
    assign-test-frequency
    assign-color 
    
    if random-float 100 < initial-infection-rate 
    [
      set infected? true
    ]
    assign-color
...
;; Asigna colores a las tortugas según su estado.
to assign-color
  if isolated?
  [
    set color gray
...
;; Asigna la tendencia a emparejarse.
to assign-coupling-tendency
  set coupling-tendency random-near average-coupling-tendency
end

;; Asigna la probabilidad de contacto con fluidos.
to assign-fluids-contact
  set fluids-contact random-near average-fluids-contact
end

;; Asigna la frecuencia de pruebas.
to assign-test-frequency
  set test-frequency random-near average-test-frequency
end

;; Genera un número aleatorio cerca de un centro dado.
...
to go
  ;; Detiene la simulación si todos están sanos o aislados.
  if all? turtles [not infected?]
  [
    stop
  ]
  if all? turtles [not infected? and not isolated?]
  [
    stop
  ]
  check-sliders
  ...
  [
    ...
    [
      ...
      [
        set isolated? false
        ifelse random 100 < 50 ;; Porcentaje de probabilidad de muerte de los aislados
        [
          die
        ]
        [
          set infected? false ;; Revive sin estar infectado
          set symptomatic? false
          set days-infected 0
        ]
      ]
  ...
  ask turtles
  [
    if not coupled? and not isolated?
    [
      move
    ]
  ]
  ...
  ask turtles [ infect ]
  ask turtles [ test ]
  ask turtles [ progress-disease ]
  ask turtles [ assign-color ]
  update-population-counts  ;; Llamar al procedimiento para actualizar los conteos.
  tick
end

;; Verifica y actualiza los sliders.
to check-sliders
  ...
  if (slider-check-3 != average-fluids-contact)
  [
    ask turtles [ assign-fluids-contact ]
    set slider-check-3 average-fluids-contact
  ]
  ...
  if (slider-check-5 != isolation-period)
  [
    set isolation-period slider-check-5
  ]
end
...
;; Empareja dos tortugas.
to couple
  let potential-partner one-of other turtles with [not coupled?]
...
;; Separa a las tortugas emparejadas.
to uncouple
  if coupled? [
    ask partner [
      set coupled? false
...
;; Progresión de la enfermedad en las tortugas.
to progress-disease
  if infected? [
    ...
    if days-infected > incubation-period + 10 [
...
;; Infecta a otras tortugas en contacto cercano.
to infect
  if infected? and symptomatic? and not isolated? [
    ask turtles in-radius 1 [
      if not infected? and random-float 100 < infection-chance [
        set infected? true
      ]
    ]
  ]
...
;; Realiza pruebas para detectar la infección.
to test
...
;; Actualiza los conteos de la población.
to update-population-counts
  set total-population count turtles
  set infected-population count turtles with [infected?]
  set symptomatic-population count turtles with [symptomatic?]
  set isolated-population count turtles with [isolated?]
end

;; Reporta el porcentaje de infectados.
to-report %infected
  ifelse any? turtles [
...
PENS
"Sanos" 1 0 -10899396 true "" "plot count turtles with [not infected?]"
"Infectados" 1 0 -2674135 true "" "plot count turtles with [known?]"
"?" 1 0 -13345367 true "" "plot count turtles with [infected?] - count turtles with [known?]"
